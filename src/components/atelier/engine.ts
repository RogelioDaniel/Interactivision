...
export class AtelierScene {
  ...
  constructor(canvas: HTMLCanvasElement, works: readonly Work[], options: SceneOptions = {}) {
    ...
    canvas.setAttribute(
      "aria-label",
      "Sala tridimensional de Interactivision. Arrastra, usa la rueda o las flechas para recorrer seis lienzos.",
    );
  ...
  regenerate(index: number, seed = Math.random() * 500 + 1) {
    ...
    if (!mesh || !work) return;
    const previous = mesh.art.material.map;
    mesh.art.material.map = canvasTexture(paintStudy(work.palette, seed), this.renderer);
    mesh.art.material.needsUpdate = true;
    previous?.dispose();
  }
  ...
  private createWorks() {
    this.works.forEach((work, index) => {
      ...
      const frame = new THREE.Mesh(new THREE.BoxGeometry(3.45, 4.28, 0.13), frameMaterial);
      ...
      const art = new THREE.Mesh(new THREE.PlaneGeometry(3.18, 4), artMaterial);
      art.position.z = 0.072;

      group.add(shadow, frame, art);
      group.rotation.z = baseTilt;
      this.world.add(group);
      this.workMeshes.push({ group, art, frame, shadow, baseY, baseTilt });
    });
  }
  ...
  private render = (now: number) => {
    ...
    this.workMeshes.forEach((mesh, index) => {
      ...
      mesh.art.material.opacity = opacity;
